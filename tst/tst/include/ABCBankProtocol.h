 ...
 * @param random [IN] 随机数
 * @param dn     [IN] 证书dn
 * @param birthCert [OUT] 通用盾出生证
 * @param idSign    [OUT] idSign 如果是国密通用盾，由”||”拼接，个数与传入的DN个数对应
 * @param P10       [OUT] P10 产生的公钥，如果是国密通用盾，由”||”拼接，个数与传入的DN个数对应。注意：对于普通二代Key，应该返回2048位rsa公钥。
 *
 * @return 错误码
 */
